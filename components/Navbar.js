...
const Navbar = () => {
  ...
  return (
    <nav className="bg-gray-950 text-white flex justify-between px-4 items-center h-13">
      <div className="logo font-bold text-lg flex jusify-center items-center">
        <Image src="/tea.gif" alt="tea" width={44} height={44} />
        <span className='cursor-pointer'><Link href="http://localhost:3000/">Get Me a Chai!</Link></span>
      </div>
      ...
      <div className="relative">
        {session && <>
            ...
            <div id="dropdown" className={`z-10 ${showdropdown? "" : "hidden"} absolute left-[125px] bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-44 dark:bg-gray-700`}>
              <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                ...
                <li>
                  <Link href="#" className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Sign out
                  </Link>
                </li>
              </ul>
            </div>
        ...
        {!session &&
          <Link href={"/login"}>
            <button type="button" className="text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">Login
            </button>
          </Link>}
      </div>
    </nav>
